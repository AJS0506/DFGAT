...
class MovieLensSmallPreprocessor():
    def __init__(self):
        ...
        self.movies_path = "./raw_data/movies.csv"
        self.ratings_path = "./raw_data/ratings.csv"
    ...
    # 4 전체 엣지셋을 dat로 저장하는 함수
    def save_edge_dat(self):
        with open("edges_small.dat", 'w', encoding='utf-8') as fp:
            for edge_data in self.edge_datas:
                src = edge_data[0]
                dst = edge_data[1]
                timestamp = edge_data[2]
                rating = edge_data[3]
                fp.write(f"{src}::{dst}::{timestamp}::{rating}\n")
    
    # 5 Train / Test로 나누어 DAT를 저장하는 함수
    def split_save_dat(self):
        ...
        with open("movielens_small_trn.dat", 'w', encoding='utf-8') as fp:
            for uid, mid, timestamp, rating in train_edges:
                fp.write(f"{uid}::{mid}::{timestamp}::{rating}\n")
        
        with open("movielens_small_val.dat", 'w', encoding='utf-8') as fp:
            for uid, mid, timestamp, rating in val_edges:
                fp.write(f"{uid}::{mid}::{timestamp}::{rating}\n")
        
        with open("movielens_small_tst.dat", 'w', encoding='utf-8') as fp:
    ...
    def preprocess(self):
        ...
        print(f"Train: {train_count} edges -> movielens_small_trn.dat")
        print(f"Val: {val_count} edges -> movielens_small_val.dat")
        print(f"Test: {test_count} edges -> movielens_small_tst.dat")

...
class DiffHeadGAT(nn.Module):
    def __init__(self, 
        ...
        # ============ Attention Score에 주입할 Pop 정보 ============
        self.pop_weight_go = pop_weight_go.to(device)
        self.pop_weight_back = pop_weight_back.to(device)

        # ============ Attention Score에 주입할 Time 정보 ============
        self.time_weight_go = time_weight_go.to(device)
        self.time_weight_back = time_weight_back.to(device)
        ...
        self.layer1_head1 = HeteroGraphConv({
            'go':   GATConv(in_feats=emb_dim, out_feats=out1_dim, num_heads=1,
                            ...
                            # # Time 정보를 Attention Score에 주입
                            # time_weight_go = self.time_weight_go, time_weight_back = self.time_weight_back),
        ...
        self.layer1_head3 = HeteroGraphConv({
            'go':   PopEncodingGATConv(in_feats=emb_dim, out_feats=out1_dim, num_heads=1,
                            ...
                            # Pop 정보를 Attention Score에 주입
                            pop_weight_go = self.pop_weight_go, pop_weight_back = self.pop_weight_back),

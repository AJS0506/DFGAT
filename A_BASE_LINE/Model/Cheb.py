...
import dgl
from dgl.nn import GraphConv, HeteroGraphConv, AGNNConv


class TwoLayerSimpleHeteroAGNNConv(nn.Module):
    def __init__(self, num_user_nodes, num_location_nodes, emb_dim, out1_dim, out2_dim, dataset):
        super(TwoLayerSimpleHeteroAGNNConv, self).__init__()
        ...
        # ============ [ Layer 1 ] ============
        # AGNNConv는 입력 차원을 유지하므로 Linear 레이어로 차원 변환
        self.linear1_user = nn.Linear(self.emb_dim, self.out1_dim)
        self.linear1_item = nn.Linear(self.emb_dim, self.out1_dim)

        # 첫 번째 HeteroGraphConv 레이어
        self.conv1 = HeteroGraphConv({
            'go': AGNNConv(),  # AGNNConv는 파라미터를 받지 않음
            'back': AGNNConv(),
        ...
        # ============ [ Layer 2 ] ============
        # Linear 레이어로 차원 변환
        self.linear2_user = nn.Linear(self.out1_dim, self.out2_dim)
        self.linear2_item = nn.Linear(self.out1_dim, self.out2_dim)

        # 두 번째 HeteroGraphConv 레이어
    ...
    def forward(self, g):
        ...
        # AGNN 후 Linear로 차원 변환
        h[self.type1_name] = self.linear1_user(h[self.type1_name])
        h[self.type2_name] = self.linear1_item(h[self.type2_name])
        # BatchNorm과 활성화
        ...
        h = self.conv2(g, h)
        # AGNN 후 Linear로 차원 변환
        h[self.type1_name] = self.linear2_user(h[self.type1_name])
        h[self.type2_name] = self.linear2_item(h[self.type2_name])
        # BatchNorm과 활성화

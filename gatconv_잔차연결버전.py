...
class GATConv(nn.Module):
    ...
    def __init__(
        ...
        # =========== 커스텀 Forward 에서 SkipConnection용 추가 ============
    ...
    def forward(self, graph, feat, edge_weight=None, my_res = None, get_attention=False):
        r"""
        ...
        with graph.local_scope():
            ...
            # CUSTOM_ RESIDUAL -> destination 노드의 추가 정보를 residual로 주입
            if my_res is not None:

                # my_res를 rst와 같은 shape로 확장 (num_heads 차원 추가)
                if rst.dim() == 3:  # [num_nodes, num_heads, out_dim]
                    # print("잔차연결 진입")
                    my_res_expanded = my_res.unsqueeze(1)  # [num_nodes, 1, encoding_dim]
                    # print(f"rst shape = {rst.shape}")
                    # print(f"my_res shape = {my_res_expanded.shape}")
                    rst = rst + my_res_expanded
                else:  # [num_nodes, out_dim]
                    rst = rst + my_res
